...
@section('content')
  <div class="max-w-4xl mx-auto">
    ...
    <h2 class="text-3xl font-bold text-gray-900 mb-6">Edit Kategori: {{ $category->nama_kategori }}
    </h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
          ...
          <div class="md:col-span-2">
            <input type="text" class="form-input text-base py-3 px-4 bg-gray-50 cursor-not-allowed"
              id="id_kategori" name="id_kategori" value="{{ $category->id }}" readonly>
            <p class="mt-1 text-sm text-gray-500">ID kategori tidak dapat diubah</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
          ...
          <div class="md:col-span-3">
            <textarea
              ...
              id="keterangan" name="keterangan" rows="3" placeholder="Masukkan keterangan kategori...">{{ old('keterangan', $category->keterangan) }}</textarea>
            @error('keterangan')
              <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
          </div>
